...
model_dir = "ConvDAE_3_1--" +  timestamp
model_path = model_root_path + model_dir

if not os.path.isdir(model_path):
   os.makedirs(model_path)

train_log_dir = 'logs/train/ConvDAE_3_1_'+timestamp
test_log_dir = 'logs/test/ConvDAE_3_1_'+timestamp
...
with tf.name_scope('outputs'):
    ...
    out1 = tf.where(tf.greater(outputs_, -0.5), outputs_, -1*tf.ones_like(outputs_))
    out2 = tf.where(tf.less(outputs_, 0.5), outputs_, tf.ones_like(outputs_))
    out3 = tf.where(tf.less_equal(outputs_, -0.5)&tf.greater_equal(outputs_, 0.5), outputs_, tf.zeros_like(outputs_))
# ## loss and Optimizer
...
#         y = abs(batch_y.reshape((-1, 28, 28, 1)))
        ...
        sess.run([out1, out2, out3],feed_dict={inputs_: x})
