...
train_log_dir = 'logs/train/SeqRNN_5_'+timestamp
test_log_dir = 'logs/test/SeqRNN_5_'+timestamp

root_path = "D:/1-Document/data/model_data/SeqRNN/"
model_dir = "SeqRNN_5--" +  timestamp
...
#y_train =  train_y[:,3,:]
y_train = train_y.transpose(0,2,1) # zzh attn
...
test_y[:,3,:] = test_y[:,3,:]+1
#y_test = test_y[:,3,:]
y_test = test_y.transpose(0,2,1) # zzh attn

#修改步长
X_train = X_train.reshape((-1,n_steps,n_inputs))
#y_train = y_train.reshape((-1,n_steps))
y_train = y_train.reshape((-1,n_steps,n_inputs)) # zzh attn

X_test = X_test.reshape((-1,n_steps,n_inputs))
#y_test = y_test.reshape((-1,n_steps))
y_test = y_test.reshape((-1,n_steps,n_inputs)) # zzh attn
...
#    y=tf.placeholder(tf.int32,[None])
    y=tf.placeholder(tf.float32,[None,n_steps,n_inputs])# zzh attn 
...
#    hiddens0, states = tf.nn.dynamic_rnn(stacked_rnn_cell, X, dtype=tf.float32) #zzh 
    ...
    # 双向LSTM
    basic_cell_fw = tf.contrib.rnn.BasicLSTMCell(num_units=n_neurons, forget_bias=0.8, activation = tf.nn.relu) #,activation = tf.nn.relu/tanh
    basic_cell_bw =tf.contrib.rnn.BasicLSTMCell(num_units=n_neurons, forget_bias=0.8, activation = tf.nn.relu) #,activation = tf.nn.relu/tanh    
    
    hiddens0, states = tf.nn.bidirectional_dynamic_rnn(basic_cell_fw, basic_cell_bw, X,  dtype=tf.float32)  ## zzh：此句暂时有错误，可能LSTM不能建立双向网络？
...
#        y_batch = y_batch.flatten() #zzh:确定一下展开顺序对不对 # zzh attn
