...
# 加载数据
path1 = r"D:\1-Codes\matlab\resource\dataset\N_MNIST_seq\full scale\N_MNIST_seq_train.mat"
path2 = r"D:\1-Codes\matlab\resource\dataset\N_MNIST_seq\full scale\N_MNIST_seq_test.mat"
...
with tf.name_scope('RNN'): 
    # 双向rnn
    basic_cell_fw = tf.contrib.rnn.BasicRNNCell(num_units=n_neurons,activation = tf.nn.relu) #,activation = tf.nn.relu/tanh
    basic_cell_bw = tf.contrib.rnn.BasicRNNCell(num_units=n_neurons,activation = tf.nn.relu) #,activation = tf.nn.relu/tanh    
    hiddens0, states = tf.nn.bidirectional_dynamic_rnn(basic_cell_fw, basic_cell_bw, X,  dtype=tf.float32)
    hiddens1 = tf.concat(hiddens0, 2)
...
#    hiddens1, states = tf.nn.dynamic_rnn(basic_cell, X, dtype=tf.float32) #zzh 
    ...
    hiddens = tf.reshape(hiddens1,[-1, 2*n_neurons]) #将batch_size*n_steps， 作为第一维，相当于新的batch_size, 参与后面的loss计算， 保证每个时间步都计算loss
...
y_test1 = y_test.flatten()   #训练时验证输入

# tensorboard
merged = tf.summary.merge_all() 

timestamp = '{:%m-%d_%H-%M/}'.format(datetime.now())
train_log_dir = 'logs/train/SeqRNN1_'+timestamp
test_log_dir = 'logs/test/SeqRNN1_'+timestamp
...
for epoch in range(n_epochs):
    ...
    if epoch%1 == 0:
        ...
        acc_test = sess.run(accuracy, feed_dict={X:X_test[0:10000,...],y:y_test1[0:640000,...]})          
...
#
##ydata = y_test.reshape((-1,n_steps))[20:10000:1000] # tmp
#ydata = np.copy(y_test)
#
#x_data_ =  np.copy(X_test)
#ypreds_ = sess.run(outputs, feed_dict={X: x_data_})
#
##preds_ = sess.run(logits, feed_dict={X: x_data_}) # tmp
##outputs = np.reshape(np.argmax(preds_,1),[-1, n_steps]) # tmp
##ypreds_ = outputs # tmp
#
##preds = np.squeeze(preds_)
#preds_data =  np.copy(x_data)  #注意复制时要deepcopy才行，*1具有这个效果
...
#
## 极性化  
##preds[:,3,:][preds[:,3,:]>=1.5] = 1
##preds[:,3,:][preds[:,3,:]<=0.5] = -1    
##preds[:,3,:][(preds[:,3,:]>0.5) & (preds[:,3,:]<1.5)] = 0    
